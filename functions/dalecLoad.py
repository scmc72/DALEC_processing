...
def load_DALEC_log(filepath, header=216, dropNA=True, longFormat=True, integerIndex=True,
    ...
    if dropDuplicates:
        # seems like sometimes we get occurences of log entries with multiple Ed readings for the same sample #....
        # this removes these. No clue what is going on here though haha
...
def load_DALEC_dir(DALEC_directory, file_names=None, **kwargs):
    ...
    if file_names is None: # if None, then load all DALEC transect (.dtf) files in the directory
        ...
        DALEC_files = [os.path.join(DALEC_directory, file) for file in file_names]
